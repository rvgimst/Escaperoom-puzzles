...
#include <FastLED.h>
// Library for LCDisplay
// See https://www.circuitbasics.com/how-to-set-up-a-keypad-on-an-arduino/?utm_source=pocket_saves 
// for useful info
...
// the liquid crystal I2C LCD (make sure on Arduino Nano to use pins: A4->SDA(SignalDAta) A5->SCL(SignalCLock)
LiquidCrystal_I2C lcd(0x27, 20, 4);
// RVG
// The different texts to be shown as hints
// TBD probably as arrays??
byte _numLCDTexts = 3;          // total available texts
byte _lcdTextIdxShown = 0;      // current text on LCD
byte _lcdTextMax = 0;           // total amount of texts to be cycled on the LCD
int _lcdTextShowtime = 3000;    // duration to show each text [ms] 10000
int _lcdTextTimeout = 1000;     // duration of empty screen between texts [ms] 5000
int _lcdAddTextTimeout = 60000; // time till next text is added in cycle
unsigned long _lastShowtime = 0;// time at which last text was shown
...
void lcdGreeting() {
  ...
  lcd.setCursor(0,3);
  lcd.print("ESCAPE From The PAST");
}

// Print a track schematic on the LCD
void lcdPrintTrack() {
  lcd.setCursor(0,0);
  lcd.print("o---O------O--*W    ");
  lcd.setCursor(0,1);
...
//            triggerSound(SND_SUBINACTIVE);
  ...
  // update the LCD track schematic to the switch positions
  // and difficulty level
  lcd.setCursor(4,0);
  lcd.print(digitalRead(switchPins[0]) ? ">" : "v");
  lcd.setCursor(11,0);
  lcd.print(digitalRead(switchPins[1]) ? ">" : "v");
  lcd.setCursor(4,3);
  lcd.print(digitalRead(switchPins[2]) ? ">" : "<");
  lcd.setCursor(11,2);
  lcd.print(digitalRead(switchPins[3]) ? ">" : "v");
  // difficulty level
  lcd.setCursor(18,0);
  lcd.print(spdChangeAllowed ? "S1" : "S0");
  lcd.setCursor(18,1);
  lcd.print(penaltiesAllowed ? "P1" : "P0");

  // Light up the extra LEDs in each track corresponding to the corresponding "score"
  for(int i=0; i<5; i++){
    for(int j=0; j<score[i]; j++){
      leds[scoreLEDs[i][j]] += colours[i];
    }
    switch(i) {
      // CRGB::White, CRGB::Green, CRGB::Blue, CRGB::Yellow, CRGB::Red
      case 0: // White
        lcd.setCursor(16,0);
        lcd.print(score[i]);
        break;
